...
**Identity adoption (2026-08-13, operator direction).** Composer treats the module name as the project's `logicalId` and every node address as that node's `logicalId` — the same identity the topology submits. Project-level resolution by that identity is [PR #230](https://github.com/prisma/composer/pull/230) (slug-first, display-name fallback, slug on create, 409 surfaced). Two things to hold in mind when the platform columns land: the wire field is called `slug` today and should ship as `logicalId` — a free rename only while #4885 and #230 are both unmerged — and the address root-segment question must be verified before stamping: core's `LowerContext.address` doc shows `auth.api` while the assembler's shows `storefront.web`, and whether lowering addresses carry the root module segment must match the topology's addresses exactly or the read-time join fails on a prefix.
