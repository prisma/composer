...
//
// Under `metadata:` rather than at the top level because the Agent Skills
// spec (agentskills.io) defines the top-level key set and reserves `metadata`
// — a string→string map — for exactly this kind of publisher extension. A
// top-level `library:` would be an undefined key that a strict runtime is
// entitled to reject.
...
export function stampSkillVersion(source: string, version: string): string {
  ...
  const stamped = metadata.replace(
    ...
    (line) => `${/^[ \t]+/.exec(line)?.[0] ?? '  '}library_version: "${version}"`,
  );
