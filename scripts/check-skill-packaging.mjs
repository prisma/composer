...
// None of those show up in a unit test — they show up in what npm uploads. So
// this check packs the package the same way the publish workflow does and
// reads the skill back out of the tarball.
//
// It also compares the packed skill byte-for-byte against the repo-root
// `skills/` tree, which is the tracked source and the tree the GitHub install
// route serves. If those two ever disagree, users on the two routes are
...
const PACKAGE = '@prisma/composer';

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const packageDir = join(repoRoot, 'packages/9-public/composer');
...
function ownedSkillNames() {
  return readdirSync(skillsRoot, { withFileTypes: true })
    ...
    .filter(
      (entry) =>
        readSkillFrontmatter(readFileSync(join(skillsRoot, entry.name, 'SKILL.md'), 'utf-8'))
          .library === PACKAGE,
    )
...
try {
  ...
  if (tarball === undefined) throw new Error(`pnpm pack produced no tarball for ${PACKAGE}`);
  execFileSync('tar', ['xzf', tarball], { cwd: work });
  const packedRoot = join(work, 'package');

  const packedVersion = JSON.parse(readFileSync(join(packedRoot, 'package.json'), 'utf-8')).version;
  ...
  for (const name of expected) {
    ...
    // install route must serve the same instructions.
