...
// the code makes that true by construction — the skill and the API it
// documents are the same artifact, updated by the same `pnpm add`. The
// separate GitHub-fetching install (`npx skills add prisma/composer`) can only
...
//
// `prepack` rather than the build: `pnpm pack` and `pnpm publish` both run it,
// so the staged copy is produced by the very act of making a tarball and can
// never be a stale build artifact restored from a turbo cache.
...
const owned = readdirSync(skillsRoot, { withFileTypes: true })
  ...
  .filter((entry) => {
    const source = readFileSync(join(skillsRoot, entry.name, 'SKILL.md'), 'utf-8');
...
for (const name of owned) {
  cpSync(join(skillsRoot, name), join(destinationRoot, name), { recursive: true });
}

process.stderr.write(`Staged ${owned.join(', ')} into ${packageName}'s tarball.\n`);
