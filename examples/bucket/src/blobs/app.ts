/**
 * A tiny blob store/serve app backed directly by a Prisma Object Store bucket.
 * It `load()`s a `BucketConfig` from a `bucket()` slot and builds an
 * `@aws-sdk/client-s3` client (path-style, region `auto`,
 * `requestChecksumCalculation: 'WHEN_REQUIRED'`) to talk to the bucket over
 ...
 *
 * `createBlobApp` returns a plain `Request → Response` handler so it runs
 * behind `Bun.serve` in the deployed service unchanged.
 */
import {
  DeleteObjectCommand,
  GetObjectCommand,
...
import type { BucketConfig } from '@prisma/composer-prisma-cloud';
...
export function createBlobApp(config: BucketConfig): (req: Request) => Promise<Response> {
  ...
  return async (req: Request): Promise<Response> => {
    ...
    if (path === '/' || path === '/health') {
      return new Response('bucket blob store — PUT/GET/DELETE /blobs/:key, GET /blobs\n', {
        status: 200,
      });
    }
