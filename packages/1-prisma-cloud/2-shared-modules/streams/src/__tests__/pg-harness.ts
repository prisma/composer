...
export async function createTestDatabase(baseUrl: string): Promise<TestDatabase> {
  const name = `streams_test_${randomUUID().replace(/-/g, '')}`;
...
export const startTestPostgres = (): TestPostgres | undefined => {
  ...
  if (initdb === undefined || pgCtl === undefined) {
    if (process.env['CI'] !== undefined) {
      throw new Error(
        'CI is set but no Postgres is available for the streams integration test: neither ' +
  ...
  const dataDir = fs.mkdtempSync(path.join(baseDir, 'prisma-compose-storage-pg-'));
