import type { Deps, Expose } from '@internal/core';
...
>(def: Parameters<typeof compute<D, I, E>>[0]): ReturnType<typeof compute<D, I, E>> {
  const node = compute<D, I, E>(def);
  const instance = new ComputeService<D, I, E>({ ...node, type: 's3-store' });
