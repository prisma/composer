/**
 * `resolveOrmConfig` — the config-path → project-facts resolution the
 * lowering does when assembling `OrmMigration` props. Loads real
 * `prisma.config.ts` fixtures via PN's config loader: the widget
...
describe('resolveOrmConfig', () => {
  ...
  test('resolveMigrationsDir stays the migrationsDir projection of resolveOrmConfig', async () => {
    expect(await resolveMigrationsDir(widgetConfig)).toBe(
      (await resolveOrmConfig(widgetConfig)).migrationsDir,
    );
  });
});
