 ...
 * contract.
 *
 * Schema is applied the faithful way — through Prisma ORM's own control
...
import type { PostgresBinding } from '../exports/orm.ts';
...
if (pg === undefined) {
  console.warn(
    '[app-cloud] skipping Prisma ORM integration test: no Postgres available. ' +
      'Set STATE_TEST_DATABASE_URL to point at one, or install initdb/pg_ctl ' +
      '(e.g. `brew install postgresql@15`) on PATH.',
  );
}

describe.skipIf(pg === undefined)('postgres hydrate — live round trip', () => {
  ...
  // type-checked.
  const contract = dataContract<WidgetContract>(widgetContractJson);
  let migrationsDir: string;
  let db: PostgresBinding<typeof contract>;
