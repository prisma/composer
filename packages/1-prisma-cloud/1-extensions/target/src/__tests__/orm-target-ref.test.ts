...
import { decideMigrationAction, OrmMigrationError, resolveTargetRef } from '../orm-migrate.ts';
...
describe('resolveTargetRef', () => {
  ...
  test('a missing named targetRef fails loudly with TARGET_REF_NOT_FOUND', async () => {
    await withTempDir(async (dir) => {
      ...
      expect((thrown as OrmMigrationError).code).toBe('TARGET_REF_NOT_FOUND');
      expect((thrown as OrmMigrationError).message).toContain('nope');
    });
  });
});
