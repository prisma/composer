 ...
 * the Alchemy stack / Prisma Cloud provisioning. Exercises `applyOrmMigration`
 * end to end in three shapes (replay-only: the pipeline never synthesizes):
 *
 * No authored graph (empty migrations dir):
 *   - empty DB → the structured refusal: OrmMigrationError
 ...
 *   - no authored path   → throws OrmMigrationError(MIGRATION_PATH_NOT_FOUND),
 *                          DB left unchanged
 *
...
if (pg === undefined) {
  console.warn(
    '[app-cloud] skipping Prisma ORM migrate integration test: no Postgres available. ' +
...
describe.skipIf(pg === undefined)(
  'applyOrmMigration — no authored graph (structured refusal)',
  () => {
    ...
    test('empty DB, empty migrations dir → refuses with both exits named; DB untouched', async () => {
      ...
      expect(thrown).toBeInstanceOf(OrmMigrationError);
      const error = thrown as OrmMigrationError;
...
describe.skipIf(pg === undefined)('applyOrmMigration — authored baseline (replay path)', () => {
  if (pg === undefined) return;
  let migrationsDir: string;
  let db: TestDatabase;
  let url: string;
  ...
  test('empty DB + committed baseline → migrate replays it and signs the marker', async () => {
    ...
    const outcome = await applyOrmMigration({
      url,
      contractJson: widgetContractJson,
      migrationsDir,
      ref,
  ...
  test('marker at a different hash with no authored path → fails, DB unchanged', async () => {
    ...
    let thrown: unknown;
    try {
      await applyOrmMigration({ url, contractJson: gadgetContractJson, migrationsDir, ref });
    } catch (e) {
      thrown = e;
    }

    expect(thrown).toBeInstanceOf(OrmMigrationError);
    expect((thrown as OrmMigrationError).code).toBe('MIGRATION_PATH_NOT_FOUND');
...
describe.skipIf(pg === undefined)(
  'applyOrmMigration — authored graph with a data invariant',
  () => {
    if (pg === undefined) return;
    let migrationsDir: string;
    ...
    test('fresh DB + ref with invariants → migrate; marker records the invariant; re-run no-ops', async () => {
      ...
      try {
        ...
        const first = await applyOrmMigration({
          url: db.url,
          contractJson: widgetContractJson,
          migrationsDir,
          ref,
        ...
        const second = await applyOrmMigration({
          url: db.url,
          contractJson: widgetContractJson,
          migrationsDir,
          ref,
    ...
    test('hash-match-but-invariant-missing (A→A data-only) → migrate stamps the invariant', async () => {
      ...
      try {
        ...
        const baseline = await applyOrmMigration({
          url: db.url,
          contractJson: widgetContractJson,
          migrationsDir,
          ref: headRef,
