...
import type { Contract, DependencyEnd, ResourceNode, ServiceNode } from '@prisma/compose';
import { dependency, freezeNode, ResourceNodeBase, string } from '@prisma/compose';
...
> extends ResourceNodeBase<C> {
  ...
  constructor(def: { name: string; contract: C; config: string; targetRef?: string }) {
    super({ name: def.name, extension: '@prisma/compose-cloud', provides: def.contract });
