...
import type { ConfigDeclaration, Contract } from '@prisma/compose';
import { configOf, hydrateSync, isNode, number, param, string } from '@prisma/compose';
...
describe('postgres({ name })', () => {
  test('returns a branded resource identity providing postgresContract; type is the contract kind', () => {
    ...
    expect(node.extension).toBe('@prisma/compose-cloud');
