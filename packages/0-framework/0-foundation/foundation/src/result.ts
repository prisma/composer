...
class ResultImpl<T, F> {
  ...
  private constructor(ok: boolean, valueOrFailure: T | F) {
    this.ok = ok;
    if (ok) {
      this._value = blindCast<
        T,
        'the private constructor is only reached through ok()/notOk(), which pass a T when ok is true'
      >(valueOrFailure);
    } else {
      this._failure = blindCast<
        F,
        'the private constructor is only reached through ok()/notOk(), which pass an F when ok is false'
      >(valueOrFailure);
    }
    Object.freeze(this);
  }
