...
ceremony as `DurableObjectNamespace` — the empty `()` lets TypeScript
...
the container. Provide `Cloudflare.layerContainer(Sandbox, …)` on
...
it. Because `layerContainer` resolves the `Sandbox` tag, **any**
