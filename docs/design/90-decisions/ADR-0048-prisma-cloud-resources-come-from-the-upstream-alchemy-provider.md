...
- **Replace the deployment on every deploy** (the pre-adoption behavior). Ships every change by brute force but gives up upstream's reuse entirely; superseded by the fingerprint, which detects changes from non-secret material only.
- **Hash environment values into the fingerprint.** A hash of a secret in plaintext state is an offline-guessing target; rejected. The fingerprint hashes only material that is non-secret by construction.
...
- The provider: the `alchemy/Prisma` module of the `alchemy` package, 2.0.0-beta.67 or later.
