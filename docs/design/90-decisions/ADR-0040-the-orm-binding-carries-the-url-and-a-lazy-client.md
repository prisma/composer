# ADR-0040: The `postgres` binding carries the raw URL and a lazy client

## Decision

`postgres(contract)`'s dependency end hydrates to a two-field binding —
...
library (say, an auth library that is not a Prisma ORM consumer) must share
...
const service = compute({
  deps: { db: postgres(appContract) },  // contract-checked, deploy-migrated
  /* … */
});
...
`PostgresBinding<C> = { readonly url: string; readonly client: Client<C> }`.
...
"framework-built client."** A `postgres` dependency hydrated directly to
...
`postgres` connection carries the same URL underneath its client. Putting
...
The Prisma ORM runtime deserializes and structurally validates
...
1. Binding shape change for existing `postgres(contract)` consumers:
...
- **Cross-kind `satisfies`** — let a `'postgres'` resource satisfy a plain
  `'postgres'` dependency, so an app declares `postgres()` and provisions
  `postgres`. Touches core's kind model to express what is really a
