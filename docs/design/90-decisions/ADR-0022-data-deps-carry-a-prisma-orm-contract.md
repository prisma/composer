# ADR-0022: Data deps carry a Prisma ORM contract; deploys migrate to its ref

## Decision

A second data primitive joins bare `postgres()`: `postgres`, a
Prisma-ORM-typed Postgres resource and dependency, shipped in
`@prisma/composer-prisma-cloud` behind its own subpath entry
(`@prisma/composer-prisma-cloud/orm`). **Prisma ORM** is Prisma's
...
The same `postgres` factory serves both ends of a data edge — a resource to
provision, and a dependency that resolves to a typed client:

```ts
// contract.ts — wrap Prisma ORM's emitted artifact into the framework's kind.
// contract.json is pure data; Contract is its emitted, branded type. Neither
// pulls in Prisma ORM's CLI or migration engine.
...
// service.ts — the DEPENDENCY end. load() hands back a typed Prisma ORM
// client, built by the framework from the contract plus the injected URL.
export default compute({
  name: 'widgets',
  deps: { db: postgres(widgetContract) },
...
// prisma.config.ts PATH — a plain string the deploy reads, never imported.
...
Prisma ORM's surface fits this shape with almost no adaptation. Its contracts
...
carries a Prisma ORM contract *can* construct the typed client from the contract
alone — contract plus URL is the client's entire input. Because "data contracts
are Prisma ORM" is a framework-level decision, Prisma ORM is blessed the way
...
- **Binding = `{ url, contractJson }`, client constructed app-side** — keeps
  ...
  Prisma ORM is framework-blessed, not an arbitrary driver.
- **Multi-contract resource declarations mapped to Prisma ORM contract
  ...
  disjointness is Prisma ORM-verified, and the space id lives on the contract
...
- [ADR-0015](ADR-0015-dependencies-resolve-to-bindings-clients-are-app-side.md) —
  amended: Prisma ORM joins the protocol-owned kinds whose binding is a client.
